...
class TestLowAveragePrice(common.TransactionCase):
    def setUp(self):
        super(TestLowAveragePrice, self).setUp()
        ...
        self.decimal_precision = self.registry('decimal.precision')
        ...
        _, product_decimal_price = self.registry('ir.model.data').get_object_reference(cr, uid, 'product', 'decimal_price')
        wf_service = netsvc.LocalService("workflow")

        self.standard_price = 0.0020
        self.order_price_unit = 0.0010
        self.available_qty = 1
        self.order_qty = 1
        self.picking_qty = 1

        # Set product price decimal precision
        self.decimal_precision.write(cr, uid,
            [product_decimal_price],
            {'digits': 4},
            context=context)

        self.product_id = self.product_product.create(cr, uid, {
            'name': 'Average product low price',
            'cost_method': 'average',
            'valuation': 'real_time',
